...
def get_uniprot_id_from_gene_symbol(symbol, email, species='all'):
    ...
    response = MyGeneInfo().query(symbol, fields='uniprot', species=species, email=email)
...
def get_pfam_entry_graphic(pfam_entry):
    with requests.Session() as session:
        response = session.get(f'http://pfam.xfam.org/protein/{pfam_entry}/graphic')
...
def get_pfam_entry_from_uniprot_id(uniprot_id):
    ...
    with requests.Session() as session:
        reader = csv.reader(session.get(url).content.decode('utf-8').splitlines(), delimiter='\t')

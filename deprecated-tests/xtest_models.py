...
class TestSnv(TestCase):
    ...
    @unittest.skipIf(pkgutil.find_loader('pyfaidx') is None, 'module pyfaidx could not be found')
    def def_set_context_from_fasta_locus(self):
        ...
        with NamedTemporaryFile('w') as infile:
            ...
            context = snv.set_context_from_fasta_locus(fn, contig='seq', position=1)
            eq_(context, 'AGT')
    ...
    def test_repr(self):
        """Test ``__repr__()``"""
        eq_(
            repr(Snv(reference='C', alternate='G')),
...
class TestSpectrum(TestCase):
    ...
    def test_invalid_settitem(self):
        ...
        assert_raises(
            KeyError, lambda snv: spectrum[snv], Snv(reference='G', alternate='A', context='G')
        )
    ...
    def test_context_weights(self):
        ...
        assert_dict_equal(dict(spectrum.context_weights), {'A': 1, 'C': 1, 'G': 1, 'T': 1})

        spectrum.context_weights['C'] += 1

        assert_dict_equal(dict(spectrum.context_weights), {'A': 1, 'C': 2, 'G': 1, 'T': 1})
    ...
    def test_density_array_with_uniform_weights(self):
        ...
        assert_array_equal(
            spectrum.density_as_array(), np.array([2 / total, 5 / total, 0, 0, 20 / total, 0])
        )
    ...
    def test_iter(self):
        ...
        spectrum = Spectrum()
        assert_list_equal(list(spectrum), list(spectrum.substitutions.items()))

    def test_str(self):
        """Test ``__str__()`` to be the same as ``__repr__()``"""
        eq_(str(Spectrum()), repr(Spectrum()))
        eq_(str(Spectrum(3)), repr(Spectrum(3)))
        eq_(str(Spectrum(3, 'pyrimidine')), repr(Spectrum(3, 'pyrimidine')))
        eq_(str(Spectrum(3, 'purine')), repr(Spectrum(3, 'purine')))

    def test_repr(self):
        """Test ``__repr__()`` with different parameter combinations"""
        eq_(repr(Spectrum()), 'Spectrum(k=1, reference_notation=None)')
        eq_(repr(Spectrum(3)), 'Spectrum(k=3, reference_notation=None)')
        eq_(repr(Spectrum(3, 'pyrimidine')), 'Spectrum(k=3, reference_notation="pyrimidine")')
        eq_(repr(Spectrum(3, 'purine')), 'Spectrum(k=3, reference_notation="purine")')
